  ...
  for (let markdown of markdowns) {
    ...
    for (let i = 0; i < tokens.length; i++) {
      ...
      if (cur?.type == "heading_open" && cur.tag == "h1") {
        if (!h1Count && (subtopics.length || tempSubtopicTitle)) {
          error(
            `Subtopic "${
              subtopics[0]?.title || tempSubtopicTitle
            }" must be enclosed inside a Topic.`
          );
          break;
        }
      ...
      if (curEl == "h1") {
        ...
        if (topicTitle.trim().length >= LengthLimits.titlesLength) {
          error(
            `Topic exceeds char length limit of ${LengthLimits.titlesLength}`
          );
          break;
        }
        continue;
      ...
      if (
        (cur?.type == "heading_open" && cur.tag == "h2") ||
        i === tokens.length - 1
      ) {
        ...
        if (tempSubtopicTitle.length) {
          ...
          if (description.trim().length >= LengthLimits.descriptionsLength) {
            error(
              `Subtopic description exceeds char length limit of ${LengthLimits.descriptionsLength}`
            );
            break;
          }
          ...
          if (tempDescription.length) {
            ...
            if (description.trim().length >= LengthLimits.descriptionsLength) {
              error(
                `Topic description exceeds char length limit of ${LengthLimits.descriptionsLength}`
              );
              break;
            }
          }
      ...
      if (curEl == "h2") {
        ...
        if (tempSubtopicTitle.trim().length >= LengthLimits.titlesLength) {
          error(
            `Subtopic exceeds char length limit of ${LengthLimits.titlesLength}`
          );
          break;
        }
        continue;
      ...
      if (cur?.type == "inline" && cur.level == 3) {
        ...
        if (factText.trim().length >= LengthLimits.factsLength) {
          error(
            `Fact exceeds char length limit of ${LengthLimits.factsLength}`
          );
          break;
        }
      ...
      if (cur?.type == "inline" && cur.level == 6 && cur.content && curFact) {
        ...
        if (question.trim().length >= LengthLimits.questionsLength) {
          error(
            `Fact question exceeds char length limit of ${LengthLimits.questionsLength}`
          );
          break;
        }
      } else if (
        ...
        if (attachment.trim().length >= LengthLimits.attachmentsLength) {
          error(
            `Fact attachment exceeds char length limit of ${LengthLimits.attachmentsLength}`
          );
          break;
        }
      }
    ...
    for (let subtopic of subtopics) {
      if (!subtopic.facts.length) {
        warn(
          `Subtopic "${subtopic.title}" contains no fact (is empty) and will be ignored`
        );
      }
    }
    ...
    for (let topic of topics) {
      const duplicatedTopics = topics.filter(
        (x) => x !== topic && x.title === topic.title
      );
    ...
    for (let subtopic of subtopics) {
      const duplicatedTopics = subtopics.filter(
        (x) => x !== subtopic && x.title === subtopic.title
      );
      if (duplicatedTopics.length) {
