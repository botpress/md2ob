...
const assertIncludes = (str1: string, str2: string, message?: string) => {
  ...
  assert(
    ...
    `Expected "${str1}" to include "${str2}".${wrap}`
  );
};
  ...
  `;
    ...
    for (let [test, ...contents] of [
      ...
      it("duplicated: " + test, () => {
        ...
        assert(
          result.warnings[0]!.message.toLowerCase().includes("duplicated") ===
            true,
          'warning must contain "duplicated"'
        );
        assert(
          result.book!.topics.length >= 1,
          "there must be at least 1 topic"
        );
        assert(
          result.book!.topics[0]!.subtopics.length >= 1,
          "there must be at least 1 subtopic"
        );
        assert(
          result.book!.topics[0]!.subtopics[0]!.facts.length >= 2,
          "there must be at least 2 facts"
        );
      });
    }
  });
...
- Fact 1`;
  ...
  `;
    ...
    assert(
      ...
      "must have 1 error " + result.errors.length
    );
...
- Fact 1`;
    const result = convert(
      content,
      content.replace("Subtopic Name", "Subtopic2 Name")
    );
    assert(result.success === false, new Error("Must fail"));
    assert(result.errors.length === 1, "must have 1 error");
